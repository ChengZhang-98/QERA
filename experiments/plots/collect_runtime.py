...
def collect_runtime(model_name, qera_scaling_mode):
    qera_config = {
    ...
    profile_outputs = evaluate_perplexity(
        ...
        num_samples=256 if qera_scaling_mode != "identity" else 4,
        progress_bar=True,
        input_device=None,
    ...
    for layer in layers_to_register_and_share:
        for pattern in patterns:
            if pattern in layer["target_layer"]:
                ...
                break
    print(
        f"layers_to_register_and_share_filtered: {layers_to_register_and_share_filtered}"
    )
    ...
    inference_and_calibrate_one_layer_elapsed = (
        time.time() - inference_and_calibrate_one_layer_start
    )

    pure_inference_elapsed = inference_elapsed
    rxx_clibration_elapsed = (
        inference_and_calibrate_one_layer_elapsed - pure_inference_elapsed
    )
    rxx_clibration_elapsed = (
        rxx_clibration_elapsed * model.config.num_hidden_layers + pure_inference_elapsed
    )
    ...
    AB_dict, mse_df = compute_AB_and_approximation_error(
        ...
        scale_dict,
        qera_config,
        move_model_back=False,
    )
    scale_calculation_elapsed = time.time() - scale_calculation_start
    scale_calculation_elapsed = (
        scale_calculation_elapsed * model.config.num_hidden_layers
    )
...
if __name__ == "__main__":
    ...
    qera_scaling_modes = ["diag", "rxx"]
    results = {}
    timestamp = datetime.datetime.now().strftime("%Y%m%d-%H%M%S")
    save_path = f"./runtime_{timestamp}.yaml"

    for qera_scaling_mode in qera_scaling_modes:
        for model_name in model_names:
            results[f"{model_name}_{qera_scaling_mode}"] = collect_runtime(
                model_name, qera_scaling_mode
            )
