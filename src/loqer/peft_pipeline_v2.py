...
def adapt_and_save_clm_model():
    ...
    parser.add_argument("adapter_init", type=str, choices=["loftq", "loqer", "loftq+loqer"])
    ...
    parser.add_argument("--loftq-num-iters", type=int, default=5, help="Default: 5")
    parser.add_argument("--bnb-quant-type", type=str, default="nf4", help="Default: nf4")
    ...
    # LoQER calibration
    scale_dict = None
    if args.adapter_init in ["loqer", "loftq+loqer"]:
    ...
    if args.adapter_init == "loftq":
        ...
        start = time.time()
        replace_lora_weights_loqer(peft_model, scale_dict=scale_dict, num_bits=args.bnb_n_bits)
        elapsed = time.time() - start + calibration_time
    elif args.adapter_init == "loftq+loqer":
        start = time.time()
        error_dict = replace_lora_weights_loftq(peft_model, num_iters=args.loftq_num_iters, num_bits=args.bnb_n_bits)
        replace_lora_weights_loqer(peft_model, scale_dict=scale_dict, num_bits=args.bnb_n_bits)
        elapsed = time.time() - start + calibration_time
